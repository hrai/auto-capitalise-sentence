...
var constants = days.concat(months);
...
},{"./utils":5,"webextension-polyfill":1}],4:[function(require,module,exports){
"use strict";
...
_webextensionPolyfill["default"].storage.local.get('sites_to_ignore').then(updateSiteIgnoreList, onError);
...
$(document).on('click', '#ignoreSiteButton', function () {
  ...
  _webextensionPolyfill["default"].tabs.query({
    ...
    $(_this).val('Site added to ignore list');
  });
});
...
_webextensionPolyfill["default"].storage.local.get('should_capitalise_i').then(function (items) {
  var shouldCapitaliseI = items.should_capitalise_i;
...
exports.getIndexOfMatchingConstantWord = getIndexOfMatchingConstantWord;
...
function getIndexOfMatchingConstantWord(text) {
  var lastWordRegex = /\b(\w+)[.?!\s]+$/;
  var match = lastWordRegex.exec(text);

  if (match) {
    var matchedWord = match[1];

    if (matchedWord != null) {
      var index = _constants.constants.findIndex(function (item) {
        return matchedWord.toLowerCase() == item.toLowerCase();
      });
...
function capitaliseText(element, shouldCapitalise, shouldCapitaliseForI, getText, setText) {
  ...
  var isLastCharAnAlphabet = lastChar.match(/[a-z]/i);
  ...
  var _getIndexOfMatchingCo = getIndexOfMatchingConstantWord(text),
      _getIndexOfMatchingCo2 = _slicedToArray(_getIndexOfMatchingCo, 2),
      index = _getIndexOfMatchingCo2[0],
      matchedWord = _getIndexOfMatchingCo2[1];

  if (index >= 0) {
    var _updatedStr2 = text.replace(matchedWord, _constants.constants[index]);

    setText(element, tagName, _updatedStr2, false);
  }
}
